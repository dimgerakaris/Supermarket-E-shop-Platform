...
// Ερώτημα για προϊόντα της κατηγορίας 'xoirino'
$sql = "SELECT id, name, image, price, description, stock FROM products WHERE category = 'xoirino'";
...
<head>
    ...
    <title>Τα Προϊόντα μας - Φρέσκα Χοιρινά</title>
...
<body>
    <section id="Products">
        <h1>Φρέσκα Χοιρινά</h1>
        <div class="products">
            <?php
            if ($result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) {
                    echo '<div class="product-item">';
                    echo '<img src="' . $row['image'] . '" alt="' . htmlspecialchars($row['name'], ENT_QUOTES) . '" class="product-img">';
                    echo '<h2>' . $row['name'] . '</h2>';
